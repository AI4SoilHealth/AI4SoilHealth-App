<template>
    ...
    <q-card flat style="display: flex; flex-direction: column" v-if="loaded" class="max-width" @keydown="handleSaveCancelKeydown">
        <q-card-actions class="q-pb-none" v-if="!parent.asForm || parent.singleRow">
            <div style="display: flex; justify-content: space-between; width: 100%; align-items: center; min-width: 600px;">
                <div>
                    <q-btn dense v-if="isAdmin" flat icon="info" color="primary" @click="parent.showRowInfo(parent.editingRow, false)">
                      <q-tooltip>{{ $t("Row info") }}</q-tooltip>
                     </q-btn> {{ parent.editMode == "add" ? $t('Adding new row') : '' }}
                </div>	
                <div>
        ...
        <div v-if="actions"><q-btn v-for="action in actions" :key="action.label" dense flat :icon="action.icon" :label="action.label" @click="action.action" /></div>
...
export default {
    ...
    props: ['parent', 'multiRow', 'rows', 'actions'],
    emits: ['cancel', 'save'],
    watch: {
            ...
            if (this.parent.isA) {
                ...
                if (value != this.oldIsAKey) {
                    ...
                    if (this.specialization) {
                        ...
                        await this.parent.loadLookups(this.isAColumns);  

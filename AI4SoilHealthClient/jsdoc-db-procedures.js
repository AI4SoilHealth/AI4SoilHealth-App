 ...
 * @returns {Object} integer idinteger custom_geometry_time_idinteger property_idcharacter_varying valuetimestamp_with_time_zone custom_geometry_time_id_valcharacter_varying property_id_val
 */
function data_custom_geometry_property_r() {
/*
CREATE OR REPLACE FUNCTION zzglc.data_custom_geometry_property_r()
 RETURNS TABLE(id integer, custom_geometry_time_id integer, property_id integer, value character varying, custom_geometry_time_id_val timestamp with time zone, property_id_val character varying)
 LANGUAGE plpgsql
AS $function$
    BEGIN

		RETURN QUERY (
    	    SELECT t.id, t.custom_geometry_time_id, t.property_id, t.value, custom_geometry_time_id.observation_time, 
    	           coalesce(property_id.name, i.name)
        	...
        	LEFT JOIN data.indicator i ON i.id = property_id.indicator_id
		);
    END $function$
...
AS $function$
	...
	RETURN (
		SELECT json_agg(t.*)
		FROM (
			SELECT r.order_no, r.name, r.path, r.component_name, 
			COALESCE (r.value, r.name) title, r.icon, r.parent, r.props, r.offline, r.public, r.active, r.id 
			FROM (
				SELECT r.order_no, r.name, r.path, r.component_name, r.offline, r.public, r.props, 
				l.value, r.icon, r.parent, r.active, r.id
				FROM meta.route r
...
function crud() {
/*
CREATE OR REPLACE FUNCTION meta.crud("SchemaName" character varying, "TableName" character varying)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    sql := '
		DROP FUNCTION IF EXISTS zzglc.' || "SchemaName" || '_' || "TableName" || '_r;
		CREATE OR REPLACE FUNCTION zzglc.' || "SchemaName" || '_' || "TableName" || '_r ()
    RETURNS TABLE (' || table_list || ref_col_list || ') AS $F$
    BEGIN
		RETURN QUERY (
    	    SELECT ' || select_column_list || ref_val_list || '
        	FROM ' || "SchemaName" || '.' || "TableName" || ' t ' || join_list || '
		);
    ...
    sql := '
		DROP FUNCTION IF EXISTS zzglc.' || "SchemaName" || '_' || "TableName" || '_c;
		CREATE OR REPLACE FUNCTION zzglc.' || "SchemaName" || '_' || "TableName" || '_c ("Json" varchar)
    ...
    sql := '
    DROP FUNCTION IF EXISTS zzglc.' || "SchemaName" || '_' || "TableName" || '_u;
    CREATE OR REPLACE FUNCTION zzglc.' || "SchemaName" || '_' || "TableName" || '_u ("Json" varchar)
    RETURNS VOID AS $F$
    DECLARE json_data json;
    BEGIN
			 json_data := "Json";
    ...
    sql := '
		DROP FUNCTION IF EXISTS zzglc.' || "SchemaName" || '_' || "TableName" || '_d;
		CREATE OR REPLACE FUNCTION zzglc.' || "SchemaName" || '_' || "TableName" || '_d 
		( "Key" ' || pk_type || ')
    RETURNS VOID AS $F$
    BEGIN
	...
	IF NOT EXISTS(SELECT  1 FROM meta.route r WHERE r.name = "TableName") THEN
		INSERT INTO meta.route (name, order_no, path, component_name, icon, parent, props, offline, public, active)
		VALUES ("TableName", 99999, '/' || "TableName", 'table', 'table_chart', NULL, format('{ "tableAPI" : "%s_%s" }',"SchemaName", "TableName")::json, FALSE, false, true) ;
	END IF;
...
AS $function$
    ...
    BEGIN
			  ...
			  json_data := "Json";
        INSERT INTO data.attribute_description (indicator_id, name, color_id, code) 
        VALUES ((json_data->>'indicator_id')::integer, (json_data->>'name')::character varying, (json_data->>'color_id')::integer, (json_data->>'code')::character varying)
        RETURNING id INTO v_id;
        RETURN v_id;
    END $function$
 ...
 * @returns {Object} integer idinteger geometry_type_idinteger person_idcharacter_varying nameboolean publictimestamp_with_time_zone time_modifiedtimestamp_with_time_zone time_createdcharacter_varying geometry_type_id_valcharacter_varying person_id_val
 */
function data_custom_geometry_r() {
/*
CREATE OR REPLACE FUNCTION zzglc.data_custom_geometry_r()
 RETURNS TABLE(id integer, geometry_type_id integer, person_id integer, name character varying, public boolean, time_modified timestamp with time zone, time_created timestamp with time zone, geometry_type_id_val character varying, person_id_val character varying)
 LANGUAGE plpgsql
AS $function$
    BEGIN

		RETURN QUERY (
    	    SELECT t.id, t.geometry_type_id, t.person_id, t.name, t.public, t.time_modified, t.time_created, geometry_type_id.name, person_id.name
        	FROM data.custom_geometry t 
        LEFT JOIN data.geometry_type geometry_type_id ON geometry_type_id.id = t.geometry_type_id
 ...
 * @returns {Object} integer idinteger indicator_idcharacter_varying nameinteger color_idcharacter_varying codecharacter_varying indicator_id_valcharacter_varying color_id_val
 */
function data_attribute_description_r() {
/*
CREATE OR REPLACE FUNCTION zzglc.data_attribute_description_r()
 RETURNS TABLE(id integer, indicator_id integer, name character varying, color_id integer, code character varying, indicator_id_val character varying, color_id_val character varying)
 LANGUAGE plpgsql
AS $function$
    BEGIN

		RETURN QUERY (
    	    SELECT t.id, t.indicator_id, t.name, t.color_id, t.code, indicator_id.name, color_id.name
...
function get_spectrum() {
/*
CREATE OR REPLACE FUNCTION data.get_spectrum("PointId" integer, "DataSourceId" integer)
 RETURNS json
 LANGUAGE plpgsql
AS $function$
BEGIN
	...
	RETURN (SELECT JSON_AGG(t.*)
			FROM (SELECT 
				  ...
				  WHERE s.point_id = "PointId"
					AND s.data_source_id = "DataSourceId") t);
...
RETURN (
	...
	'descriptions', (SELECT json_agg(t.*) 
		 FROM (SELECT indicator_id, attribute_description.id value, name label 
 ...
 * @function save_file
 * @memberof data
 * @description Save or update file (image) attached to custom geometry
 * @param {integer} PersonId
...
function save_file() {
/*
CREATE OR REPLACE FUNCTION data.save_file("PersonId" integer, "Id" integer, "CustomGeometryId" integer, "FileName" character varying, "Extension" character varying, "MimeType" character varying, "Compass" integer, "Lat" double precision, "Lon" double precision)
 RETURNS integer
 LANGUAGE plpgsql
AS $function$ 
	DECLARE v_id INT;
...
--  END IF;
    ...
    ELSE
        UPDATE data.custom_geometry_file SET
            name = "FileName", extension = "Extension", mime_type = "MimeType", compass = "Compass", geometry = v_geom
        WHERE id = v_id AND EXISTS(
            SELECT 1 FROM data.custom_geometry g WHERE g.id = "CustomGeometryId" AND g.person_id = "PersonId"
        );
		GET DIAGNOSTICS v_RowCountInt = ROW_COUNT;
		IF v_RowCountInt = 0 THEN
			SELECT meta.raise_error(4);
 ...
 * @param {integer} PersonId
 * @param {integer} LangId
 * @returns {Object} bigint idcharacter_varying table_schemacharacter_varying table_name
 */
function list_tables() {
/*
CREATE OR REPLACE FUNCTION meta.list_tables("Params" text, "PersonId" integer, "LangId" integer)
 RETURNS TABLE(id bigint, table_schema character varying, table_name character varying)
 LANGUAGE plpgsql
AS $function$
DECLARE db VARCHAR;
DECLARE sql VARCHAR;
...
AS $function$
    ...
    BEGIN
			  ...
			  json_data := "Json";
        INSERT INTO data.custom_geometry (geometry_type_id, person_id, name, public, time_modified, time_created) 
        VALUES ((json_data->>'geometry_type_id')::integer, (json_data->>'person_id')::integer, (json_data->>'name')::character varying, (json_data->>'public')::boolean, (json_data->>'time_modified')::timestamp with time zone, (json_data->>'time_created')::timestamp with time zone)
        RETURNING id INTO v_id;
        RETURN v_id;
    END $function$
...
CREATE OR REPLACE FUNCTION data.get_indicator_values("IndicatorId" integer, "DataSourceId" integer)
 RETURNS TABLE(lon double precision, lat double precision, value real, date date, color_code character varying, point_key character varying, nuts_0 character, description character varying, point_name character varying, point_id integer, sample integer, has_spectrum boolean)
 LANGUAGE plpgsql
AS $function$
...
BEGIN
	...
	IF v_numerical = false THEN
			...
			RETURN QUERY
				SELECT p.lon, p.lat, null::float4 value, m.date, ic.code, p.point_key, n.cntr_code, d.name, p.name point_name, p.id point_id, p.sample
			       , EXISTS(SELECT 1 FROM data.spectrum s WHERE s.point_id = id AND s.data_source_id = "DataSourceId") has_spectrum
	...
	ELSE
		RETURN QUERY
			SELECT t.lon, t.lat, avg(t.value)::float4 value, t.date, t.color_code, t.point_key, t.cntr_code, t.description, t.point_name, t.point_id, t.sample
			       , EXISTS(SELECT 1 FROM data.spectrum s WHERE s.point_id = t.point_id AND s.data_source_id = "DataSourceId") has_spectrum
			FROM (
				SELECT p.lon, p.lat, m.value, m.date, ic.color_code, p.point_key, n.cntr_code, null::varchar description, d.name depth_name, p.name point_name, p.id point_id, p.sample,
			...
			GROUP BY t.lon, t.lat, t.date, t.color_code, t.point_key, t.cntr_code, t.description, t.point_name, t.point_id, t.sample
			ORDER BY value
			; 
	END IF;
END 
 ...
 * @memberof data
 * @description Create or update custom geometry time
 * @param {integer} PersonId
 * @param {integer} CustomGeometryId
 * @param {integer} Id
...
CREATE OR REPLACE FUNCTION auth.login("PersonId" integer, "IP" character varying)
 ...
 LANGUAGE plpgsql
AS $function$
DECLARE v_person json;
DECLARE v_agreement json;
BEGIN

	INSERT INTO auth.log(person_id, ip)
	VALUES ("PersonId", "IP");
	
	SELECT row_to_json(t) INTO v_person FROM (
		SELECT t.*, EXISTS(SELECT 1 FROM auth.person_role_public r WHERE r.person_id = t.id AND r.role_public_id = 1) is_admin
		FROM general.person t
		WHERE id = "PersonId"
	) t;

	IF (v_person->>'agreed' IS NULL) THEN
        SELECT row_to_json(a) INTO v_agreement FROM meta.app a;
        v_person := (v_person::jsonb || v_agreement::jsonb)::json;
	END IF;
	RETURN v_person;
 ...
 * @memberof data
 * @description Create or update custom geometry
 * @param {integer} PersonId
 * @param {integer} Id
 * @param {integer} GeometryTypeId
 * @param {character_varying} Name
 ...
 * @memberof data
 * @description Update custom geometry by id with new geometry
 * @param {integer} PersonId
 * @param {integer} Id
 * @param {integer} FileId
 * @param {text} Feature
 ...
 * @returns {Object} integer idtext agreementtext privacy_policytext terms_of_service
 */
function meta_app_r() {
/*
CREATE OR REPLACE FUNCTION zzglc.meta_app_r()
 RETURNS TABLE(id integer, agreement text, privacy_policy text, terms_of_service text)
 LANGUAGE plpgsql
AS $function$
    BEGIN

		RETURN QUERY (
    	    SELECT t.id, t.agreement, t.privacy_policy, t.terms_of_service
        	FROM meta.app t 
		);
    END $function$
...
AS $function$
    ...
    BEGIN
        ...
        geometry_type_id = (json_data->>'geometry_type_id')::integer, 
 ...
 * @returns {Object} integer idinteger point_idinteger indicator_idinteger data_source_iddate dateinteger attribute_description_idinteger depth_idcharacter_varying point_id_valcharacter_varying indicator_id_valcharacter_varying data_source_id_valcharacter_varying attribute_description_id_valcharacter_varying depth_id_val
 */
function data_attribute_r() {
/*
CREATE OR REPLACE FUNCTION zzglc.data_attribute_r()
 RETURNS TABLE(id integer, point_id integer, indicator_id integer, data_source_id integer, date date, attribute_description_id integer, depth_id integer, point_id_val character varying, indicator_id_val character varying, data_source_id_val character varying, attribute_description_id_val character varying, depth_id_val character varying)
 LANGUAGE plpgsql
AS $function$
    BEGIN
...
BEGIN
	...
	INSERT INTO meta.route (name, order_no, path, component_name, icon, parent, props, public, active, offline)
	SELECT name || ' cloned', order_no, path, component_name, icon, parent, props, public, active, offline
	FROM meta.route
	WHERE id = "Id"
	RETURNING id INTO v_id;
...
CREATE OR REPLACE FUNCTION zzglc.meta_news_r()
 RETURNS TABLE(id integer, name character varying, active boolean, time_modified timestamp with time zone, user_modified integer, time_created timestamp with time zone, user_modified_val character varying)
 LANGUAGE plpgsql
AS $function$
BEGIN
 ...
 * @returns {Object} integer idcharacter_varying proc_namecharacter_varying sheetcharacter_varying preprocessinteger data_source_iddate datecharacter_varying data_source_id_val
 */
function data_import_procedure_r() {
/*
CREATE OR REPLACE FUNCTION zzglc.data_import_procedure_r()
 RETURNS TABLE(id integer, proc_name character varying, sheet character varying, preprocess character varying, data_source_id integer, date date, data_source_id_val character varying)
 LANGUAGE plpgsql
AS $function$
    BEGIN
...
BEGIN
    ...
    FROM (				
				...
				SELECT json_build_object(
					'type', 'Feature',
					'id', 'g' || id::VARCHAR,
					'geometry', g.geometry, -- st_asgeojson(geometry)::json,
 ...
 * @property {character_varying} text
 * @property {character_varying} extended_text
 * @property {timestamptz_with_time_zone} time_modified
 * @property {integer} user_modified
 * @property {timestamptz_with_time_zone} time_created
 * @property {boolean} active
 * @property {character_varying} title
...
CREATE OR REPLACE FUNCTION zzglc.meta_news_lang_r()
 RETURNS TABLE(id integer, lang_id integer, title character varying, text character varying, extended_text character varying, time_modified timestamp with time zone, user_modified integer, time_created timestamp with time zone, active boolean, news_id integer, lang_id_val character varying, user_modified_val character varying, news_id_val character varying)
 LANGUAGE plpgsql
AS $function$
BEGIN
    ...
    RETURN QUERY
    SELECT
    t.id, t.lang_id, t.title, t.text, t.extended_text, t.time_modified, t.user_modified, t.time_created, t.active, t.news_id
...
AS $function$
    ...
    BEGIN
			  ...
			  json_data := "Json";
        INSERT INTO meta.app (agreement, privacy_policy, terms_of_service) 
        VALUES ((json_data->>'agreement')::text, (json_data->>'privacy_policy')::text, (json_data->>'terms_of_service')::text)
        RETURNING id INTO v_id;
        RETURN v_id;
    END $function$
...
BEGIN
	...
	UPDATE general.person p SET agreed = now() WHERE id = "PersonId";
END $function$
...
BEGIN
  ...
  v_sql := E'SELECT json_agg(t.*) FROM (\n SELECT COALESCE(p.sample::varchar, p.name, p.point_key) name, ids[2]::int elev, d.name depth, ids[4]::date date, ';

  SELECT string_agg('t."' || ind.name || '"', ', ') INTO v_part
  FROM ind ORDER BY 1;

  v_part := v_part || ', s.wavelengths, s.values';
...
BEGIN
	...
	SELECT json_build_object('id', id, 'geometry_type_id', geometry_type_id, 'name', name, 'public', public,
                             'lon', st_x(geometry), 'lat', st_y(geometry))
	INTO v_general
	FROM (
	    SELECT id, geometry_type_id, name, public, st_centroid(geometry) geometry FROM data.custom_geometry	                                                                            
	    WHERE id = "Id" AND person_id = "PersonId"
    ) t;
	...
	IF v_general IS NOT NULL THEN
		...
		SELECT json_agg(t.*) INTO v_files
			FROM (
					SELECT f.id, f.custom_geometry_id, f.name, f.mime_type, f.time_modified, f.extension, f.compass, st_x(f.geometry) lon, st_y(f.geometry) lat
 ...
 * @returns {json}
 */
function get_nuts_boundaries() {
/*
CREATE OR REPLACE FUNCTION data.get_nuts_boundaries("Level" integer, "Srid" integer, "Zoom" integer, "MinX" double precision, "MinY" double precision, "MaxX" double precision, "MaxY" double precision, "IndicatorId" integer, "DataSourceId" integer)
 RETURNS json
 LANGUAGE plpgsql
AS $function$
